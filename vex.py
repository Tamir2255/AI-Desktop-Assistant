...
def understand_command(text):
    ...
    if "summarize the following" in text.lower():
        content = text.split("following:")[-1].strip() if "following:" in text else ""
...
class VexInterface:
    def __init__(self):
        ...
        self.current_tab = "chat" # chat, write, read, history
        ...
        self.theme_colors = {
            ...
            "active_btn": "#30363D",
            "vex_header": "#BC8CFF",
            "ring_in": "#0D1117", "ring1": "#00E5FF", "ring2": "#7000FF", "ring_out": "#161B22"
        }
    ...
    def build_ui(self):
        ...
        self.nav_chat = create_nav_btn("💬", "chat")
        self.nav_write = create_nav_btn("✍️", "write")
        self.nav_read = create_nav_btn("📖", "read")
        self.nav_hist = create_nav_btn("📜", "history")
        ...
        self.voice_btn = self.make_button(self.btn_row, "🎙", self.listen_once, bg=self.theme_colors["accent"], fg=self.theme_colors["bg"])
    ...
    def switch_tab(self, tab_name):
        ...
        # Reset visual states
        for btn in [self.nav_chat, self.nav_write, self.nav_read, self.nav_hist]:
    ...
    def show_write_panel(self):
        if not hasattr(self, 'write_frame'):
            self.write_frame = tk.Frame(self.content_frame, bg=self.theme_colors["bg"])
            tk.Label(self.write_frame, text="WRITE ASSISTANT", fg=self.theme_colors["accent"], bg=self.theme_colors["bg"], font=("Segoe UI", 12, "bold")).pack(pady=20)
            ...
            for label, prompt in templates:
                btn = self.make_button(self.write_frame, label, lambda p=prompt: self.apply_template(p))
    ...
    def show_read_panel(self):
        if not hasattr(self, 'read_frame'):
            self.read_frame = tk.Frame(self.content_frame, bg=self.theme_colors["bg"])
            tk.Label(self.read_frame, text="READ & ANALYZE", fg=self.theme_colors["accent"], bg=self.theme_colors["bg"], font=("Segoe UI", 12, "bold")).pack(pady=20)
            tk.Label(self.read_frame, text="Highlight text in any app, copy it (Ctrl+C),\nthen use the buttons below:", fg="gray", bg=self.theme_colors["bg"], font=("Segoe UI", 9)).pack(pady=5)
            
            actions = [
                ("🔍 Explain Selection", "Explain this highlighted text: "),
                ("📊 Summarize Selection", "Summarize the following: "),
                ("🌐 Translate Selection", "Translate this to English: "),
                ("📄 Analyze File Content", "Read and explain the file: ")
            ]
            for label, prompt in actions:
                btn = self.make_button(self.read_frame, label, lambda p=prompt: self.analyze_clipboard(p))
                btn.pack(fill="x", pady=5, padx=20)
    ...
    def open_name_settings(self):
        ...
        # Name Setting
        tk.Label(dialog, text="What should Vex call you?", bg=self.theme_colors["bg"], fg=self.theme_colors["text"],
        tk.Label(scroll_content, text="What should Vex call you?", bg=self.theme_colors["bg"], fg=self.theme_colors["text"],
                 font=("Segoe UI", 10)).pack(pady=(20, 5))
        
        global USER_NAME
        name_entry = tk.Entry(dialog, width=30, bg=self.theme_colors["entry"], fg=self.theme_colors["text"],
        ...
        tk.Label(dialog, text="What is my name?", bg=self.theme_colors["bg"], fg=self.theme_colors["text"], font=("Segoe UI", 10)).pack(pady=(10, 5))
        assistant_entry = tk.Entry(dialog, width=30, bg=self.theme_colors["entry"], fg=self.theme_colors["text"], relief="flat", font=("Segoe UI", 10))
        ...
        tk.Label(dialog, text="Core Design:", bg=self.theme_colors["bg"], fg=self.theme_colors["text"], font=("Segoe UI", 10)).pack(pady=(10, 5))
        ...
        design_menu = tk.OptionMenu(dialog, design_var, *designs)
        ...
        # Birthday Setting
        tk.Label(dialog, text="Birthday (MM-DD):", bg=self.theme_colors["bg"], fg=self.theme_colors["text"],
        ...
        bday_entry = tk.Entry(dialog, width=30, bg=self.theme_colors["entry"], fg=self.theme_colors["text"],
        ...
        tk.Checkbutton(dialog, text="Silent Startup", variable=silent_var, 
        ...
        def save_settings():
            ...
            if not CONFIG["silent_startup"]:
                ...
                emit_output(f"{CONFIG.get('assistant_name', 'Vex')}: Personalization updated.")
            dialog.destroy()
    ...
    def _add_output(self, text):
        ...
        elif text.startswith("Vex: "):
            msg = text.replace("Vex: ", "")
            self.output.insert("end", " ● VEX \n", "vex_header")
