...
# Suppress dependency warnings caused by library version mismatches
import warnings
...
# Optional "smart mode" providers. These are completely optional - the
# assistant is fully functional without any of them, using only live web
# lookups. If the person adds their own key in API Settings, these get used.
try:
    import openai
except ImportError:
    openai = None
...
# ----------------------------------------------------------------------------
# PORTABLE PATHS
# No path on this page is tied to a specific machine, developer account, or
# project folder, so the app runs correctly for anyone who installs it.
# ----------------------------------------------------------------------------
def _is_frozen():
...
def _base_dir():
    """Directory the script/exe is actually running from."""
    if _is_frozen():
        return Path(sys.executable).resolve().parent
    return Path(__file__).resolve().parent


APP_FOLDER_NAME = "AI Desktop Assistant"

# Per-user, writable application data folder (works on any Windows account,
# and degrades gracefully on macOS/Linux for development/testing).
if os.name == "nt":
    APP_DATA_DIR = Path(os.environ.get("APPDATA", str(Path.home()))) / APP_FOLDER_NAME
else:
    APP_DATA_DIR = Path.home() / f".{APP_FOLDER_NAME.lower().replace(' ', '_')}"
...
DOCUMENT_OUTPUT_DIR = HOME_DIRECTORY / "Desktop" / "AI Assistant Output"
...
LEARNING_FILE = APP_DATA_DIR / "learning.json"
...
def load_config():
    ...
    try:
        with open(CONFIG_FILE, 'r') as f:
            data = json.load(f)
        # Fill in any keys added by newer versions without wiping user data.
        changed = False
...
SELECTED_VOICE = CONFIG.get("selected_voice", "Default")
...
def ask_external_brain(query):
    """Optional smart-mode enhancement. Only runs if the person has entered
    their own OpenAI or Gemini key in API Settings AND turned Smart Mode on.
    The assistant works completely fine without this - it's an opt-in boost,
    never a requirement."""
    if not CONFIG.get("smart_mode"):
        return None
...
def search_web(query, detailed=False, return_text=False):
    """Answers are gathered from live web sources by default. If the person has
    opted into Smart Mode with their own key, that is tried first for a more
    conversational answer; otherwise everything runs fully self-contained."""
    ...
    try:
        ...
        with opener.open(req) as response:
            ...
            if return_text:
                ...
                if detailed:
                    emit_output(f"{ASSISTANT_NAME}: Gathering live sources for complete content...")
                    collected_text = []
                ...
                if not answer and not detailed:
                    emit_output(f"{ASSISTANT_NAME}: DuckDuckGo direct answer not available. Checking Wikipedia...")
                ...
                if not answer and not detailed:
                    emit_output(f"{ASSISTANT_NAME}: Checking deeper sources...")
                    for i in range(min(len(links), 2)):
...
def understand_command(text):
    ...
    if "translate this to english" in text.lower():
        content = text.split(":", 1)[-1].strip() if ":" in text else ""
        return "analyze_text", ("translate", content)
    ...
    def run_download():
        try:
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ydl.download([url])
            speak("Download complete! I've saved the video to your output folder.")
        except Exception as e:
...
def handle_graphing(expression):
    ...
    try:
        ...
        p = plot(expr, show=False, title=f"Graph of {expression}", line_color='#22D3EE')
...
def handle_math(expression):
    ...
    try:
        ...
        result = parsed_expr.evalf() if parsed_expr.is_number and "nsimplify" not in clean_expr else parsed_expr
...
# ----------------------------------------------------------------------------
# THEMES - a full palette system, switchable live from Settings > Themes
# ----------------------------------------------------------------------------
THEMES = {
    "Aurora": {
        "bg": "#0B0E14", "bg_alt": "#10141D", "header": "#131826", "panel": "#161C2B",
        "accent": "#22D3EE", "accent2": "#8B5CF6", "accent_warm": "#F472B6",
        "text": "#ECEFF4", "text_dim": "#8590A6", "entry": "#1B2233", "btn": "#1C2333",
        "btn_fg": "#7DE8FF", "active_btn": "#26314A", "vex_header": "#B18CFF",
        "danger": "#FB7185", "success": "#34D399",
        "ring_out": "#1B2233", "ring2": "#8B5CF6",
    },
    "Midnight Ember": {
        "bg": "#120B0A", "bg_alt": "#1A100E", "header": "#20140F", "panel": "#241612",
        "accent": "#FB923C", "accent2": "#EF4444", "accent_warm": "#FCD34D",
        "text": "#FBEEE6", "text_dim": "#A8887C", "entry": "#2A1815", "btn": "#2C1A16",
        "btn_fg": "#FFB27A", "active_btn": "#3A211C", "vex_header": "#F97316",
        "danger": "#F87171", "success": "#4ADE80",
        "ring_out": "#2A1815", "ring2": "#EF4444",
    },
    "Emerald Forest": {
        "bg": "#08130F", "bg_alt": "#0D1B15", "header": "#0F2019", "panel": "#122A20",
        "accent": "#34D399", "accent2": "#10B981", "accent_warm": "#A3E635",
        "text": "#E8FBF3", "text_dim": "#7CA898", "entry": "#153226", "btn": "#153226",
        "btn_fg": "#6EE7B7", "active_btn": "#1B3E2E", "vex_header": "#4ADE80",
        "danger": "#F87171", "success": "#34D399",
        "ring_out": "#153226", "ring2": "#10B981",
    },
    "Solar Gold": {
        "bg": "#14100A", "bg_alt": "#1C160D", "header": "#221A0F", "panel": "#271E12",
        "accent": "#FBBF24", "accent2": "#F59E0B", "accent_warm": "#FDE68A",
        "text": "#FBF3E3", "text_dim": "#B0996F", "entry": "#2D2413", "btn": "#2D2413",
        "btn_fg": "#FCD34D", "active_btn": "#3B3018", "vex_header": "#FBBF24",
        "danger": "#F87171", "success": "#84CC16",
        "ring_out": "#2D2413", "ring2": "#F59E0B",
    },
    "Rose Quartz": {
        "bg": "#150A11", "bg_alt": "#1E0E17", "header": "#24101B", "panel": "#2A1220",
        "accent": "#F472B6", "accent2": "#C026D3", "accent_warm": "#FDA4AF",
        "text": "#FCE9F3", "text_dim": "#B085A0", "entry": "#301425", "btn": "#301425",
        "btn_fg": "#F9A8D4", "active_btn": "#3D1830", "vex_header": "#E879F9",
        "danger": "#FB7185", "success": "#34D399",
        "ring_out": "#301425", "ring2": "#C026D3",
    },
    "Ocean Depths": {
        "bg": "#08111A", "bg_alt": "#0B1826", "header": "#0E1E30", "panel": "#112438",
        "accent": "#38BDF8", "accent2": "#3B82F6", "accent_warm": "#67E8F9",
        "text": "#E7F3FC", "text_dim": "#7C9AB0", "entry": "#132A40", "btn": "#132A40",
        "btn_fg": "#7DD3FC", "active_btn": "#173350", "vex_header": "#60A5FA",
        "danger": "#F87171", "success": "#2DD4BF",
        "ring_out": "#132A40", "ring2": "#3B82F6",
    },
    "Monochrome": {
        "bg": "#0E0E10", "bg_alt": "#151517", "header": "#1A1A1D", "panel": "#1E1E21",
        "accent": "#E5E7EB", "accent2": "#9CA3AF", "accent_warm": "#F3F4F6",
        "text": "#F4F4F5", "text_dim": "#8B8B92", "entry": "#222226", "btn": "#222226",
        "btn_fg": "#D4D4D8", "active_btn": "#2B2B30", "vex_header": "#E5E7EB",
        "danger": "#F87171", "success": "#A3E635",
        "ring_out": "#222226", "ring2": "#9CA3AF",
    },
}
...
class VexInterface:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title(f"{ASSISTANT_NAME} Sidebar")
        ...
        self.root.configure(bg="#0B0E14", highlightthickness=0, bd=0)
        ...
        theme_name = CONFIG.get("theme_name", "Aurora")
        self.theme_colors = dict(THEMES.get(theme_name, THEMES["Aurora"]))

        self.learning_data = {}
        if LEARNING_FILE.exists():
            try:
                self.learning_data = json.loads(LEARNING_FILE.read_text())
            except Exception:
                pass
        ...
        self.apply_rounded_corners(self.sidebar_width, self.bar_height, 28)
    ...
    # ------------------------------------------------------------------
    # LOGO - an original "A + I" mark (never a sparkle/star), fully vector
    # drawn on a Canvas so it scales cleanly at any size.
    # ------------------------------------------------------------------
    def draw_ai_logo(self, canvas, cx, cy, size, primary, secondary):
        top = (cx, cy - size)
        left = (cx - size * 0.85, cy + size * 0.85)
        right = (cx + size * 0.85, cy + size * 0.85)
        stroke = max(2, int(size * 0.16))

        canvas.create_line(*top, *left, fill=primary, width=stroke, capstyle="round", joinstyle="round")
        canvas.create_line(*top, *right, fill=primary, width=stroke, capstyle="round", joinstyle="round")

        bar_y = cy + size * 0.15
        t = (bar_y - top[1]) / (left[1] - top[1])
        left_x = top[0] + t * (left[0] - top[0])
        right_x = top[0] + t * (right[0] - top[0])
        canvas.create_line(left_x, bar_y, right_x, bar_y, fill=primary, width=max(2, int(size * 0.13)), capstyle="round")

        i_top = cy - size * 1.15
        i_bottom = cy + size * 1.15
        cap_w = size * 0.42
        canvas.create_line(cx, i_top, cx, i_bottom, fill=secondary, width=stroke, capstyle="round")
        canvas.create_line(cx - cap_w, i_top, cx + cap_w, i_top, fill=secondary, width=stroke, capstyle="round")
        canvas.create_line(cx - cap_w, i_bottom, cx + cap_w, i_bottom, fill=secondary, width=stroke, capstyle="round")

    def make_logo_canvas(self, parent, px, bg, primary, secondary, command=None):
        canvas = tk.Canvas(parent, width=px, height=px, bg=bg, highlightthickness=0, cursor="hand2" if command else "")
        cx = cy = px / 2
        size = px * 0.24
        self.draw_ai_logo(canvas, cx, cy, size, primary, secondary)
        if command:
    ...
    def build_ui(self):
        ...
        self.nav_bar = tk.Frame(self.main_container, bg=c["header"], width=64, bd=0, highlightthickness=0)
        self.nav_bar.pack(side="left", fill="y")
        self.nav_bar.pack_propagate(False)

        brand_wrap = tk.Frame(self.nav_bar, bg=c["header"])
        brand_wrap.pack(pady=(16, 6))
        self.brand_logo = self.make_logo_canvas(brand_wrap, 34, c["header"], c["accent"], c["accent2"])
        self.brand_logo.pack()
        tk.Frame(self.nav_bar, bg=c["accent2"], height=2).pack(fill="x", padx=14, pady=(0, 10))
        ...
        def create_nav_btn(icon, label, tab_name):
            ...
            btn = tk.Button(wrap, text=icon, font=("Segoe UI", 15),
                             bg=c["header"], fg=c["text_dim"],
                             activebackground=c["btn"], activeforeground=c["accent"],
                             relief="flat", bd=0, cursor="hand2",
                             command=lambda: self.switch_tab(tab_name))
            btn.pack(fill="x", ipady=4)
            cap = tk.Label(wrap, text=label, font=("Segoe UI", 7, "bold"),
                            bg=c["header"], fg=c["text_dim"])
            cap.pack(fill="x")

            def on_enter(e):
                if self.current_tab != tab_name:
                    btn.config(bg=c["btn"]); cap.config(bg=c["btn"]); wrap.config(bg=c["btn"])

            def on_leave(e):
                if self.current_tab != tab_name:
                    btn.config(bg=c["header"]); cap.config(bg=c["header"]); wrap.config(bg=c["header"])

            for w in (btn, cap, wrap):
                w.bind("<Enter>", on_enter)
                w.bind("<Leave>", on_leave)

            self.nav_buttons[tab_name] = (btn, cap, wrap)
            return btn

        create_nav_btn("💬", "CHAT", "chat")
        create_nav_btn("✍️", "WRITE", "write")
        create_nav_btn("📖", "READ", "read")
        create_nav_btn("📜", "LOG", "history")
        ...
        self.title_bar = tk.Frame(self.body_container, bg=c["header"], height=38, bd=0, highlightthickness=0)
        self.title_bar.pack(fill="x", side="top")

        self.title_accent = tk.Frame(self.title_bar, bg=c["accent"], height=3)
        self.title_accent.pack(fill="x", side="bottom")

        title_left = tk.Frame(self.title_bar, bg=c["header"])
        title_left.pack(side="left", padx=10)
        self.title_logo = self.make_logo_canvas(title_left, 20, c["header"], c["accent"], c["text"])
        self.title_logo.pack(side="left", padx=(0, 6))
        self.title_label = tk.Label(title_left, text=ASSISTANT_NAME.upper(), fg=c["text"], bg=c["header"],
                                     font=("Segoe UI", 10, "bold"))
        self.title_label.pack(side="left")

        self.close_btn = tk.Button(self.title_bar, text="✕", command=self.root.destroy, bg=c["header"], fg=c["danger"],
                                    activebackground=c["header"], relief="flat", font=("Arial", 10, "bold"), bd=0, cursor="hand2")
        self.close_btn.pack(side="right", padx=6)

        self.settings_btn = tk.Button(self.title_bar, text="⚙", command=self.show_settings_menu, bg=c["header"], fg=c["text_dim"],
                                       activebackground=c["header"], relief="flat", font=("Arial", 11), bd=0, cursor="hand2")
        self.settings_btn.pack(side="right", padx=4)

        self.min_btn = tk.Button(self.title_bar, text="—", command=self.toggle_sidebar, bg=c["header"], fg=c["text_dim"],
                                  activebackground=c["header"], relief="flat", font=("Arial", 10, "bold"), bd=0, cursor="hand2")
        ...
        self.canvas = tk.Canvas(self.content_frame, width=150, height=150, bg=c["bg"], highlightthickness=0)
        self.canvas.pack(pady=(14, 4))

        self.status = tk.Label(self.content_frame, text=f"{ASSISTANT_NAME.upper()} ONLINE",
                                fg=c["vex_header"], bg=c["bg"], font=("Segoe UI", 12, "bold"))
        self.status.pack(pady=(0, 8))

        self.chip_row = tk.Frame(self.content_frame, bg=c["bg"])
        self.chip_row.pack(fill="x", pady=(0, 8))

        chips = [
            ("🔎 Explain", "Explain this highlighted text: "),
            ("📝 Summarize", "Summarize this into 5 bullets: "),
            ("🌐 Translate", "Translate this to English: "),
            ("✉️ Email", "Write a professional email about: "),
        ]
        for label, prompt in chips:
            chip = tk.Button(self.chip_row, text=label, font=("Segoe UI", 8, "bold"),
                              bg=c["btn"], fg=c["btn_fg"], activebackground=c["active_btn"],
                              relief="flat", bd=0, cursor="hand2", padx=8, pady=4,
                              command=lambda p=prompt: self.use_quick_chip(p))
            chip.pack(side="left", expand=True, fill="x", padx=2)

        self.input_container = tk.Frame(self.content_frame, bg=c["bg"], bd=0, highlightthickness=0)
        self.input_container.pack(fill="x", side="bottom", pady=(0, 22))

        self.entry_border = tk.Frame(self.input_container, bg=c["accent2"], highlightthickness=0, bd=0)
        ...
        self.placeholder = f"Ask {ASSISTANT_NAME} anything..."
        ...
        self.submit_btn = self.make_button(self.btn_row, "➤", self.submit_command, bg=c["accent2"], fg=c["text"])
        self.submit_btn.pack(side="left", fill="x", expand=True, padx=2)
        self.voice_btn = self.make_button(self.btn_row, "🎙", self.listen_once, bg=c["accent"], fg=c["bg"])
        ...
        self.make_button(self.btn_row, "📋", self.copy_last_answer).pack(side="left", fill="x", expand=True, padx=2)
        self.make_button(self.btn_row, "?", self.run_help).pack(side="left", padx=2)

        self.output_frame = tk.Frame(self.content_frame, bg=c["bg"])
        self.output_frame.pack(fill="both", expand=True, pady=(0, 10))
        self.output_frame.grid_columnconfigure(0, weight=1)
        self.output_frame.grid_rowconfigure(0, weight=1)

        self.scrollbar = tk.Scrollbar(self.output_frame, width=8, bg=c["btn"], troughcolor=c["bg"],
        ...
        self.output = tk.Text(self.output_frame, bg=c["bg"], fg=c["text"], insertbackground=c["accent"],
                               relief="flat", wrap="word", font=("Segoe UI", 10), padx=10, pady=10,
        ...
        self.output.tag_configure("user_header", foreground=c["accent2"], font=("Segoe UI", 9, "bold"), spacing1=15, justify="right")
        self.output.tag_configure("vex_header", foreground=c["vex_header"], font=("Segoe UI", 9, "bold"), spacing1=15)
        self.output.tag_configure("message_body", foreground=c["text"], spacing1=5, spacing3=5, lmargin1=15, lmargin2=15)
        self.output.tag_configure("user_body", foreground=c["text"], background=c["panel"], spacing1=5, spacing3=5, rmargin=15, justify="right")
        ...
        self.output.tag_configure("link", foreground=c["accent"], underline=1)
        ...
        self.float_btn = self.make_logo_canvas(self.root, 60, c["accent"], c["bg"], c["header"], command=self.toggle_sidebar)

    def use_quick_chip(self, prompt):
    ...
    def switch_tab(self, tab_name):
        ...
        for name, (btn, cap, wrap) in self.nav_buttons.items():
            active = name == tab_name
            color_bg = c["btn"] if active else c["header"]
            color_fg = c["accent"] if active else c["text_dim"]
            btn.config(bg=color_bg, fg=color_fg)
            cap.config(bg=color_bg, fg=color_fg)
            wrap.config(bg=color_bg)
    ...
    def _make_action_card(self, parent, icon, title, subtitle, command):
        c = self.theme_colors
        card = tk.Frame(parent, bg=c["panel"], bd=0, highlightthickness=1, highlightbackground=c["entry"])
        card.pack(fill="x", pady=6, padx=18)

        inner = tk.Frame(card, bg=c["panel"], cursor="hand2")
        inner.pack(fill="x", padx=12, pady=10)

        icon_lbl = tk.Label(inner, text=icon, font=("Segoe UI", 16), bg=c["panel"], fg=c["accent"])
        icon_lbl.pack(side="left", padx=(0, 10))

        text_col = tk.Frame(inner, bg=c["panel"])
        text_col.pack(side="left", fill="x", expand=True)
        tk.Label(text_col, text=title, font=("Segoe UI", 10, "bold"), bg=c["panel"], fg=c["text"], anchor="w").pack(fill="x")
        tk.Label(text_col, text=subtitle, font=("Segoe UI", 8), bg=c["panel"], fg=c["text_dim"], anchor="w", wraplength=self.sidebar_width - 140, justify="left").pack(fill="x")

        for w in (card, inner, icon_lbl, text_col) + tuple(text_col.winfo_children()):
        ...
        def on_leave(e):
            card.config(highlightbackground=c["entry"])
    ...
    def show_write_panel(self):
        c = self.theme_colors
        if hasattr(self, 'write_frame'):
            self.write_frame.destroy()
        self.write_frame = tk.Frame(self.content_frame, bg=c["bg"])
        header = tk.Frame(self.write_frame, bg=c["bg"])
        header.pack(fill="x", pady=(18, 6))
        tk.Label(header, text="✍️  Write Assistant", fg=c["text"], bg=c["bg"], font=("Segoe UI", 13, "bold")).pack(anchor="w", padx=18)
        tk.Label(header, text="Generated locally from live web sources - no external AI required.",
                 fg=c["text_dim"], bg=c["bg"], font=("Segoe UI", 8), wraplength=self.sidebar_width - 60, justify="left").pack(anchor="w", padx=18, pady=(2, 0))

        templates = [
            ("📧", "Professional Email", "Draft a polished, ready-to-send email.", "Write a professional email about: "),
            ("✨", "Improve My Text", "Tighten wording and fix tone.", "Improve this text: "),
            ("📝", "Short Summary", "Condense text into quick bullet points.", "Summarize this into 3 bullets: "),
            ("🚀", "Action Plan", "Turn a goal into concrete next steps.", "Create an action plan for: "),
            ("📄", "Essay / Research", "Full researched write-up saved to Word.", "Write an essay on: "),
        ]
        for icon, title, subtitle, prompt in templates:
            self._make_action_card(self.write_frame, icon, title, subtitle, lambda p=prompt: self.apply_template(p))
        self.write_frame.pack(fill="both", expand=True)

    def show_read_panel(self):
        c = self.theme_colors
        if hasattr(self, 'read_frame'):
            self.read_frame.destroy()
        self.read_frame = tk.Frame(self.content_frame, bg=c["bg"])
        header = tk.Frame(self.read_frame, bg=c["bg"])
        header.pack(fill="x", pady=(18, 6))
        tk.Label(header, text="📖  Read & Analyze", fg=c["text"], bg=c["bg"], font=("Segoe UI", 13, "bold")).pack(anchor="w", padx=18)
        tk.Label(header, text="Highlight text anywhere, copy it (Ctrl+C), then tap an action below.",
                 fg=c["text_dim"], bg=c["bg"], font=("Segoe UI", 8), wraplength=self.sidebar_width - 60, justify="left").pack(anchor="w", padx=18, pady=(2, 0))

        actions = [
            ("🔍", "Explain Selection", "Get a clear breakdown of the copied text.", "Explain this highlighted text: "),
            ("📊", "Summarize Selection", "Turn a long passage into key points.", "Summarize the following: "),
            ("🌐", "Translate Selection", "Translate the copied text to English.", "Translate this to English: "),
            ("📄", "Analyze File Content", "Read and explain a file by name.", "Read and explain the file: "),
        ]
        for icon, title, subtitle, prompt in actions:
            self._make_action_card(self.read_frame, icon, title, subtitle, lambda p=prompt: self.analyze_clipboard(p))
    ...
    def show_history_view(self):
        ...
        if hasattr(self, 'history_view_frame'):
            self.history_view_frame.destroy()

        self.history_view_frame = tk.Frame(self.content_frame, bg=c["bg"])
        header = tk.Frame(self.history_view_frame, bg=c["bg"])
        header.pack(fill="x", pady=(18, 6))
        tk.Label(header, text="📜  Activity Log", fg=c["text"], bg=c["bg"], font=("Segoe UI", 13, "bold")).pack(anchor="w", padx=18)

        clear_btn = tk.Button(header, text="Clear", command=self.clear_history, font=("Segoe UI", 8, "bold"),
                               bg=c["btn"], fg=c["danger"], activebackground=c["active_btn"], relief="flat", bd=0, cursor="hand2", padx=8, pady=2)
        clear_btn.pack(anchor="e", padx=18, pady=(0, 4))

        body = tk.Frame(self.history_view_frame, bg=c["bg"])
        body.pack(fill="both", expand=True, padx=10)

        self.hist_canvas = tk.Canvas(body, bg=c["bg"], highlightthickness=0)
        self.hist_scroll = tk.Scrollbar(body, orient="vertical", command=self.hist_canvas.yview)
        self.hist_list_container = tk.Frame(self.hist_canvas, bg=c["bg"])

        self.hist_canvas.create_window((0, 0), window=self.hist_list_container, anchor="nw", width=self.sidebar_width - 90)
        ...
        if not self.history:
            tk.Label(self.hist_list_container, text="No history yet", fg=c["text_dim"], bg=c["bg"]).pack(pady=20)
        else:
            for item in reversed(self.history):
                f = tk.Frame(self.hist_list_container, bg=c["panel"], pady=2, highlightthickness=1, highlightbackground=c["entry"])
                f.pack(fill="x", pady=3, padx=5)
                btn = tk.Button(f, text=f"↺ {item}", command=lambda i=item: self.reuse_history_item(i),
                                bg=c["panel"], fg=c["text"], activebackground=c["active_btn"],
                                relief="flat", anchor="w", font=("Segoe UI", 9), wraplength=self.sidebar_width - 110, justify="left", cursor="hand2")
                btn.pack(fill="x", side="left", expand=True, padx=6, pady=4)
    ...
    def toggle_sidebar(self):
        ...
        if not self.is_collapsed:
            ...
            self.apply_rounded_corners(60, 60, 60)
    ...
    def toggle_calc_mode(self):
        ...
        if self.calc_mode:
            self.calc_btn.config(fg="#ffffff", bg=c["accent2"])
            self.status.config(text="CALCULATOR MODE", fg=c["accent_warm"])
            self.entry.delete(0, "end")
            self.entry.insert(0, "Enter math problem...")
            self.entry.config(fg=c["text_dim"])
        else:
            self.calc_btn.config(fg=c["btn_fg"], bg=c["btn"])
            self.status.config(text=f"{ASSISTANT_NAME.upper()} ONLINE", fg=c["vex_header"])
            self.entry.delete(0, "end")
    ...
    def show_settings_menu(self):
        c = self.theme_colors
        menu = tk.Menu(self.root, tearoff=0, bg=c["header"], fg=c["text"],
                       activebackground=c["accent"], activeforeground=c["bg"], font=("Segoe UI", 10))

        theme_menu = tk.Menu(menu, tearoff=0, bg=c["header"], fg=c["text"],
    ...
    def set_theme(self, theme_name):
        ...
        save_config(CONFIG)
        self.theme_colors = dict(THEMES[theme_name])

        was_collapsed = self.is_collapsed
        self.main_container.destroy()
        self.float_btn.destroy()
        for attr in ("write_frame", "read_frame", "history_view_frame"):
    ...
    def open_api_settings(self):
        ...
        tk.Label(dialog, text="🔑 Bring Your Own AI Keys", bg=c["bg"], fg=c["text"], font=("Segoe UI", 11, "bold")).pack(pady=(18, 2))
        tk.Label(dialog, text=f"{ASSISTANT_NAME} works fully on its own using live web search.\nAdding a key below is optional and only powers Smart Mode.",
                 bg=c["bg"], fg=c["text_dim"], font=("Segoe UI", 8), justify="center").pack(pady=(0, 14))

        tk.Label(dialog, text="OpenAI API Key:", bg=c["bg"], fg=c["text"], font=("Segoe UI", 10)).pack(pady=(6, 4))
        openai_entry = tk.Entry(dialog, width=38, show='*', bg=c["entry"], fg=c["text"],
                                 insertbackground=c["accent"], relief="flat", bd=0, highlightthickness=0, font=("Segoe UI", 10))
        openai_entry.insert(0, CONFIG.get("openai_api_key", ""))
        openai_entry.pack(pady=4)

        tk.Label(dialog, text="Gemini API Key:", bg=c["bg"], fg=c["text"], font=("Segoe UI", 10)).pack(pady=(10, 4))
        gemini_entry = tk.Entry(dialog, width=38, show='*', bg=c["entry"], fg=c["text"],
                                 insertbackground=c["accent"], relief="flat", bd=0, highlightthickness=0, font=("Segoe UI", 10))
        gemini_entry.insert(0, CONFIG.get("gemini_api_key", ""))
        gemini_entry.pack(pady=4)

        tk.Label(dialog, text="ElevenLabs API Key (voice):", bg=c["bg"], fg=c["text"], font=("Segoe UI", 10)).pack(pady=(10, 4))
        eleven_entry = tk.Entry(dialog, width=38, show='*', bg=c["entry"], fg=c["text"],
                                 insertbackground=c["accent"], relief="flat", bd=0, highlightthickness=0, font=("Segoe UI", 10))
        eleven_entry.insert(0, CONFIG.get("elevenlabs_api_key", ""))
        eleven_entry.pack(pady=4)

        smart_var = tk.BooleanVar(value=CONFIG.get("smart_mode", False))
        tk.Checkbutton(dialog, text="Enable Smart Mode (use my key when available)", variable=smart_var,
                       bg=c["bg"], fg=c["text"], selectcolor=c["header"], activebackground=c["bg"],
                       font=("Segoe UI", 9), bd=0).pack(pady=(14, 6))

        def save_keys():
            global ELEVENLABS_API_KEY
            CONFIG["openai_api_key"] = openai_entry.get().strip()
            CONFIG["gemini_api_key"] = gemini_entry.get().strip()
            CONFIG["elevenlabs_api_key"] = eleven_entry.get().strip()
            CONFIG["smart_mode"] = smart_var.get()
            ELEVENLABS_API_KEY = CONFIG["elevenlabs_api_key"]
            save_config(CONFIG)
            speak("API settings updated.")
            dialog.destroy()
        ...
        tk.Button(btn_frame, text="Save", command=save_keys, bg=c["accent"], fg="#031014",
                  activebackground=c["accent2"], relief="flat", font=("Segoe UI", 10, "bold"),
                  padx=15, pady=5, bd=0, highlightthickness=0, cursor="hand2").pack(side="left", padx=10)
        tk.Button(btn_frame, text="Cancel", command=cancel_dialog, bg=c["btn"], fg=c["text"],
                  activebackground=c["active_btn"], relief="flat", font=("Segoe UI", 10),
                  padx=15, pady=5, bd=0, highlightthickness=0, cursor="hand2").pack(side="left", padx=10)
    ...
    def open_name_settings(self):
        ...
        tk.Label(scroll_content, text="What should the assistant call you?", bg=c["bg"], fg=c["text"], font=("Segoe UI", 10)).pack(pady=(20, 5))
        name_entry = tk.Entry(scroll_content, width=30, bg=c["entry"], fg=c["text"], insertbackground=c["accent"],
                               relief="flat", bd=0, highlightthickness=0, font=("Segoe UI", 10))
        name_entry.insert(0, USER_NAME)
        name_entry.pack(pady=5)

        tk.Label(scroll_content, text="What should the assistant be called?", bg=c["bg"], fg=c["text"], font=("Segoe UI", 10)).pack(pady=(10, 5))
        assistant_entry = tk.Entry(scroll_content, width=30, bg=c["entry"], fg=c["text"], relief="flat", font=("Segoe UI", 10))
        assistant_entry.insert(0, CONFIG.get("assistant_name", "Nova"))
        assistant_entry.pack(pady=5)

        tk.Label(scroll_content, text="Core Design:", bg=c["bg"], fg=c["text"], font=("Segoe UI", 10)).pack(pady=(10, 5))
        ...
        design_menu.config(bg=c["btn"], fg=c["text"], relief="flat")
        design_menu["menu"].config(bg=c["header"], fg=c["text"])
        design_menu.pack(pady=5)

        tk.Label(scroll_content, text="Birthday (MM-DD):", bg=c["bg"], fg=c["text"], font=("Segoe UI", 10)).pack(pady=(10, 5))
        bday_entry = tk.Entry(scroll_content, width=30, bg=c["entry"], fg=c["text"], insertbackground=c["accent"],
                               relief="flat", bd=0, highlightthickness=0, font=("Segoe UI", 10))
        ...
        def save_settings():
            ...
            self.root.title(f"{ASSISTANT_NAME} Sidebar")
            self.title_label.config(text=ASSISTANT_NAME.upper())
            self.placeholder = f"Ask {ASSISTANT_NAME} anything..."
            if self.entry.get() == "" or self.entry.cget("fg") == self.theme_colors["text_dim"]:
                self.entry.delete(0, "end")
                self.entry.insert(0, self.placeholder)
                self.entry.config(fg=self.theme_colors["text_dim"])
            self.status.config(text=f"{ASSISTANT_NAME.upper()} ONLINE")
        ...
        tk.Button(btn_frame, text="Save", command=save_settings, bg=c["accent"], fg="#031014",
                  activebackground=c["accent2"], relief="flat", font=("Segoe UI", 10, "bold"),
                  padx=15, pady=5, bd=0, highlightthickness=0, cursor="hand2").pack(side="left", padx=10)
        tk.Button(btn_frame, text="Cancel", command=cancel_dialog, bg=c["btn"], fg=c["text"],
                  activebackground=c["active_btn"], relief="flat", font=("Segoe UI", 10),
    ...
    def start_drag(self, event):
        if isinstance(event.widget, (tk.Button, tk.Scrollbar, tk.Text, tk.Entry, tk.Canvas)):
    ...
    def make_button(self, parent, text, command, bg=None, fg=None):
        ...
        btn = tk.Button(
            ...
            bd=0, highlightthickness=0, font=("Segoe UI", 10, "bold"), padx=8, pady=6, cursor="hand2",
        )
    ...
    def _add_output(self, text):
        ...
        elif text.startswith(f"{ASSISTANT_NAME}: "):
            msg = text.replace(f"{ASSISTANT_NAME}: ", "", 1)
            self.output.insert("end", f" ● {ASSISTANT_NAME.upper()} \n", "vex_header")
            self.output.insert("end", f" {msg} \n\n", "message_body")
        else:
    ...
    def animate_core(self):
        ...
        main_color = c["accent_warm"] if self.is_searching else c["accent"]
        alt_color = c["text"] if self.is_searching else c["ring2"]

        if design == "Classic":
            base_radius = 28 + (self.pulse if self.is_speaking else (math.sin(self.pulse / 10) * 4 + 4))
            rings = [(base_radius + 38, c["ring_out"]), (base_radius + 20, alt_color), (base_radius + 6, main_color)]
            ...
            rad = 34 + math.sin(self.pulse / 5) * 12
            ...
            self.canvas.create_oval(center - 25, center - 25, center + 25, center + 25, outline=main_color, width=3)
            for i in range(3):
                angle = (self.pulse * 5 + i * 120) * (math.pi / 180)
                ox, oy = center + 46 * math.cos(angle), center + 46 * math.sin(angle)
            ...
            for i in range(4):
                ...
                rad = 42 + (math.sin(self.pulse / 5) * 8)
            ...
            for x in range(30, 120, 5):
                y = center + math.sin((x + self.pulse * 10) / 20) * 16
                points.extend([x, y])
            self.canvas.create_line(points, fill=main_color, smooth=True, width=3)
            self.canvas.create_oval(center - 38, center - 38, center + 38, center + 38, outline=alt_color, width=2)

        core_rad = 25 + (self.pulse % 10 if self.is_speaking else 0)
        self.canvas.create_oval(center - core_rad, center - core_rad, center + core_rad, center + core_rad, fill=c["bg"], outline=main_color, width=2)
        self.draw_ai_logo(self.canvas, center, center, core_rad * 0.5, main_color if not self.is_searching else c["text"], c["text"] if not self.is_searching else main_color)
    ...
    def startup(self):
        ...
        if last_run != today:
            ...
            if is_birthday: msg += f" And Happy Birthday, {USER_NAME if USER_NAME else 'there'}!"
    ...
    def copy_last_answer(self):
        ...
        if start_idx != -1:
            ...
            if final_answer:
                ...
                self.root.after(1500, lambda: self.status.config(text=original_text, fg=self.theme_colors["success"]))
    ...
    def process_command(self, user_input):
        ...
        if user_input_lower in ["hello", "hello there", "hey", "hi there", "hi", "wassup", "sup", "yo"]:
            respond(f"Hello! {ASSISTANT_NAME} here. What's up? How can I help you today?")
            return
        ...
        creator_line = CONFIG.get("creator_name", "").strip()
        name_answer = f"I'm {ASSISTANT_NAME}, your personal desktop AI assistant."
        if creator_line:
            name_answer += f" I was made by {creator_line}."

        farewell = "Goodbye, have a great day!"
        note = CONFIG.get("farewell_note", "").strip()
        if note:
            farewell += f" {note}"

        responses = {
            "how are you": "I'm doing great, thanks for asking!",
            "what is your name": name_answer,
            "bye": farewell,
            "haha": "Why are you laughing? That wasn't funny.",
            "wow": "Yeah I know, pretty cool right?",
        }
        ...
        if intent == "analyze_text":
            ...
            if not content:
                ...
                return
            verbs = {"explain": "Analyzing", "summarize": "Summarizing", "translate": "Translating"}
            respond(f"{verbs.get(mode, 'Analyzing')} that text for you now...")
        ...
        if intent == "create_doc":
            ...
            name = f"Export_{doc_type}_{datetime.now().strftime('%H%M%S')}"
...
def create_shortcuts():
    ...
    shortcut_targets = [
        Path.home() / "Desktop" / f"{ASSISTANT_NAME}.lnk",
        Path(os.environ.get("APPDATA", "")) / "Microsoft" / "Windows" / "Start Menu" / "Programs" / f"{ASSISTANT_NAME}.lnk",
    ]
    ...
    for shortcut_path in shortcut_targets:
        ...
        try:
            ...
            shortcut.Description = f"Launch {ASSISTANT_NAME} assistant"
            shortcut.Save()
        except OSError:
            continue
